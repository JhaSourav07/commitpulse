...
describe('BackgroundRefresh - Massive Data Sets and Extreme High Bounds Scaling (Variation 2)', () => {
  ...
  it('tracks 50,000 unique active jobs and verifies dedup prevents double-triggering', () => {
    ...
    const callsForUser0 = getFullDashboardData.mock.calls.filter(
      (call) => call[0] === 'tracked_user_0'
    ).length;
  ...
  it('handles 5k triggers where getFullDashboardData resolves synchronously via microtask queue', async () => {
    ...
    vi.mocked(getFullDashboardData).mockImplementation(() => {
      return Promise.resolve({} as Awaited<ReturnType<typeof getFullDashboardData>>);
    });
  ...
  it('maintains isStale correctness near the threshold boundary with sub-millisecond precision', () => {
    ...
    const barelyStale = new Date(now - THRESHOLD - 1).toISOString();
    expect(service.isStale(barelyStale)).toBe(true);

    const barelyFresh = new Date(now - THRESHOLD + 100).toISOString();
    expect(service.isStale(barelyFresh)).toBe(false);

    const atThreshold = new Date(now - THRESHOLD).toISOString();
    expect(service.isStale(atThreshold)).toBe(false);

    for (let ms = 1; ms <= 100; ms++) {
      const ts = new Date(now - THRESHOLD - ms).toISOString();
