...
describe('RefreshPolicy - Edge Cases & Empty/Missing Inputs', () => {
  ...
  it('Singleton Reset Stability: clears all state and restores defaults on reset', () => {
    ...
    // Verify cooldown is restored to default (15 * 60 * 1000 = 900000ms)
    policy.recordRefresh('user-a');
    // It should be around 900000 right after recording (allow minor execution delay)
    const remaining = policy.getRemainingCooldown('user-a');
    expect(remaining).toBeLessThanOrEqual(900000);
    expect(remaining).toBeGreaterThanOrEqual(890000);
  });
});
