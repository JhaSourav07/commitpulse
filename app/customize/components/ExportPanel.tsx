...
export function ExportPanel({
  ...
  const handleCopyMarkdown = async () => {
    ...
    const markdownString = `![CommitPulse](${targetUrl})`;
    try {
  ...
  const handleCopyHtml = async () => {
    ...
    const htmlString = `<img src="${targetUrl}" alt="CommitPulse" />`;
    try {
        ...
        <div className="flex items-center gap-3">
          ...
          <>
            ...
            <button
              ...
              aria-label="copy markdown export snippet to clipboard"
              aria-describedby="export-copy-status"
              onClick={handleCopyMarkdown}
              disabled={!hasUsername}
              className={`relative inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-200 ${
                !hasUsername
                  ? 'bg-gray-200/90 border border-black/10 text-gray-500 cursor-not-allowed dark:bg-white/10 dark:border-white/10'
                  : markdownCopied || copied
                    ? 'bg-emerald-500/15 border border-emerald-500/30 text-emerald-700 dark:text-emerald-400'
                    : 'bg-gray-200/90 border border-black/10 text-gray-800 hover:bg-gray-300/80 hover:scale-[1.03] active:scale-[0.98] dark:bg-white/10 dark:border-white/10 dark:text-gray-200 dark:hover:bg-white/20'
              }`}
            >
              {markdownCopied || copied ? 'Copied!' : 'Copy Markdown'}
            </button>
            ...
            <button
              type="button"
              id="copy-html-btn"
              aria-label="copy html markup to clipboard"
              aria-describedby="export-copy-status"
            ...
            >
              {htmlCopied || copied ? 'Copied!' : 'Copy HTML'}
            </button>

            {/* Accessible Live Region for Vitest Pipeline Verification */}
            <div id="export-copy-status" role="status" aria-live="polite" className="sr-only">
              {markdownCopied || copied
