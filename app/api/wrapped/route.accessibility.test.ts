...
describe('ApiWrappedRoute Accessibility (WCAG / ARIA compliance)', () => {
  beforeEach(() => {
    ...
    vi.mocked(getWrappedData).mockResolvedValue({
      profile: { username: 'octocat', name: 'The Octocat', avatarUrl: '' },
      score: 100,
      calendar: { totalContributions: 0, weeks: [], colors: [], months: [] },
      streakStats: {
        currentStreak: 0,
        peakStreak: 0,
        longestStreak: 0,
    ...
    it('5. sanitizes user inputs inside SVG accessibility tags to prevent malformed XML or script injections', async () => {
      ...
      vi.mocked(getWrappedData).mockResolvedValueOnce(maliciousStats as any);
