...
vi.mock('@/lib/github', () => ({
  fetchGitHubContributions: vi.fn().mockResolvedValue({ totalContributions: 100, syncStatus: 'SUCCESS' }),
}));
...
describe('AppTemplate Mock Integrations', () => {
  beforeEach(() => {
    // Fix 1: Resets both call history and custom temporary implementations cleanly
    vi.resetAllMocks();
  });
  ...
  it('renders pending state overlays during service loading paths', () => {
    // Fix 2: Evaluates the actual Template component state branch instead of a local function wrapper
    render(
      <Template>
    ...
    // Fix 4: Implements a specific, semantic DOM presence matching check
  ...
  it('triggers correct fallback procedures during fake endpoint timeout blocks', async () => {
    ...
    vi.mocked(github.fetchGitHubContributions).mockRejectedValueOnce(new Error('TIMEOUT_GATEWAY_RESET'));

    let interfaceFallbackMessage = '';
    try {
      await github.fetchGitHubContributions('stale-user');
    } catch (err) {
      // Fix 3: Safely catches and asserts specific error signatures, satisfying the unused variable rule
    ...
    expect(interfaceFallbackMessage).toBe('Connection timed out. Loading local offline landscape profiles.');
  });
