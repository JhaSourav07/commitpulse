...
// 2. Intercept SocialsSection to inject our failing child when we want to trigger a crash
vi.mock('./SocialsSection', async () => {
  const actual = await vi.importActual<typeof import('./SocialsSection')>('./SocialsSection');
  return {
    ...actual,
    SocialsSection: ({ forceCrash }: { forceCrash?: boolean }) =>
      forceCrash ? (
        <ChildThatThrows />
      ) : (
        <div data-testid="socials-section-success">Socials Section Hydrated Successfully</div>
      ),
  };
});
...
describe('SocialsSection: Hydration Stability, Exception Safety & Error Fallbacks', () => {
  ...
  it('Test 1: should maintain Hydration Stability and render without crashing on initial load', () => {
    ...
    expect(screen.getByTestId('socials-section-success')).toBeInTheDocument();
  });
  ...
  it('Test 3: should render a clean error recovery UI instead of white-screening', () => {
    ...
    expect(screen.queryByTestId('socials-section-success')).not.toBeInTheDocument();
  });
