...
describe('DashboardClient', () => {
  ...
  it("awards 'Most Consistent' badge to the profile with the higher peak streak in compare mode", async () => {
    const initialData = {
      ...mockInitialData,
      stats: { ...mockInitialData.stats, peakStreak: 50 },
    };
    const secondUserData = {
      ...mockSecondData,
      stats: { ...mockSecondData.stats, peakStreak: 10 },
    };
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockImplementation(() =>
        Promise.resolve({
          ok: true,
          json: () => Promise.resolve(secondUserData),
        })
      )
    );

    render(<DashboardClient initialData={initialData} username="Shivangi1515" />);
    ...
    const firstProfileCard = profileCards.find((card) => within(card).queryByText('Shivangi'));
