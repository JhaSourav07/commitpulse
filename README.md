...
| `lang`            | `string`  | No         | `auto` (detect)                | Language code for labels (`en`, `es`, `hi`, `fr`, `pt`, `ko`, `ja`, `de`, `zh`, `ta`). If omitted, it automatically detects the viewer's language from their browser settings. |
