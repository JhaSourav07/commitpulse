...
function renderFooter(
  ...
  const centerX = width / 2;
  return `
  ${!params.hide_stats ? renderStatsSection(stats, labels, s, params, width) : ''}
  ${!params.hide_title ? `<text x="${centerX}" y="${s(50)}" text-anchor="middle" class="title">${truncateUsername(safeUser).toUpperCase()}</text>` : ''}
  <rect
    x="${s(100)}"
    y="${s(60)}"
    width="${s(400)}"
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" fill="none" role="img">
  ...
  ${renderFooter(stats, params, labels, safeUser, accent, sf, W)}
</svg>`;
}
...
>
  ...
  ${!params.hide_stats ? renderStatsSection(stats, labels, s, params, W) : ''}
${
  !params.hide_title
    ? `<text x="${centerX}" y="${s(50)}" text-anchor="middle" class="title">${truncateUsername(safeUser).toUpperCase()}</text>`
    : ''
}
