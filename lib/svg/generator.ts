...
function renderHeader(
  ...
  return `
  <title>CommitPulse ${entity} Stats for ${truncateUsername(safeUser)}</title>
  <desc>
    ${truncateUsername(safeUser)} has ${stats.totalContributions} ${unit} and a longest streak of ${stats.longestStreak} days.
  </desc>
  ${renderDefs(sf)}`;
}
